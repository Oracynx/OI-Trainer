  ...
  checkAndHandleTalentLoss(student) {
    ...
    registerDefaultTalents(game, utils){
      ...
      this.registerTalent({
        ...
        this.registerTalent({
          name: '原题机（真）',
          description: '在NOI系列比赛（不包括省选）中所有属性增加200%，但有30%概率被发现获得0分，学校罚款50%当前金额，声誉降低至20%。在IOI中属性降低20%，且有60%的概率被发现，成绩0分，其他中国选手保留最高分一位，剩余中国选手（除中国最高分，天赋持有本人）成绩减半。',
          color: '#FF0000',
          prob: 0.02,
          kind: 'negative',
          beneficial: false,
                  ...
                  if(state){
                    ...
                    try{
                      if(typeof window !== 'undefined' && window.game){
                        const currentFunds = Number(window.game.funds || 0);
                        const fine = currentFunds * 0.5;
                        window.game.funds = Math.max(0, currentFunds - fine);
                        
                        // 声誉降低至20%
                        window.game.reputation = 20;
                        
                        // 记录事件
                        if(window.pushEvent){
                          window.pushEvent({
                            name: '原题机被发现',
                            description: `${student.name} 在${contestName}中被发现使用原题机！成绩清零，学校罚款${fine}元，声誉降至20%`,
