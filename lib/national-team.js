...
(function(global){
  ...
  function calculateNationalTeamQualification() {
    ...
    // 晋级：前4名且总分超过晋级线
    let qualified = [];
    for(let i = 0; i < Math.min(4, totalScores.length); i++){
    ...
    html += `</tbody></table>`;
    html += `<div style="margin-top:8px;font-size:13px;color:#666;">晋级线：${passLine.toFixed(0)} 分（总分50%），最多4人晋级</div>`;
    html += `</div>`;
